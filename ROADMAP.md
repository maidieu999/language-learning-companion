...
3. **Get document** — `GET /documents/:id` (current user only)
4. **Update document** — `PATCH /documents/:id` with optional `title` and/or `content`; re-chunks and re-embeds when `content` changes
5. **Delete document** — `DELETE /documents/:id` removes the document, its chunks, and embeddings
6. **Chunk** — sliding window (1000 characters, 200 overlap) via `ChunkingService`
7. **Embed** — Gemini `gemini-embedding-001` (1536 dimensions, `RETRIEVAL_DOCUMENT`) via `AiService`
8. **Store** — rows in `Document`, `Chunk`, and `Embedding` (`vector(1536)`)
...
3. **Edit material** — inline form calls `PATCH /documents/:id` (re-indexes when content changes)
4. **Delete material** — `DELETE /documents/:id` with confirmation
